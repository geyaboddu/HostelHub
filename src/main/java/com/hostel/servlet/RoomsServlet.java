    ...
    protected void doGet(HttpServletRequest request,
        ...
        try {
            ...
            String sql =
                  ...
                  + "r.capacity, "
                  + "(SELECT COUNT(DISTINCT a.bed_number) "
                  + " FROM allocations a "
                  + " WHERE a.room_id = r.room_id "
                  + " AND a.status = 'Approved' "
                  + " AND a.bed_number IS NOT NULL) AS occupied, "
                  + "r.ac, "
                  + "CASE "
                  + "WHEN (SELECT COUNT(DISTINCT a.bed_number) "
                  + "      FROM allocations a "
                  + "      WHERE a.room_id = r.room_id "
                  + "      AND a.status = 'Approved' "
                  + "      AND a.bed_number IS NOT NULL) >= r.capacity "
                  + "THEN 'Full' "
                  + "ELSE 'Available' "
                  + "END AS status "
                  + "FROM rooms r "
